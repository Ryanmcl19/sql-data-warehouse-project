...
BEGIN
	...
	BEGIN TRY
		...
		-- Loading the silver.crm_cust_info table
		...
		-- Loading the silver.crm_prd_info table
		SET @start_time = GETDATE();
		...
		SELECT
			...
			SUBSTRING(prd_key, 7, LEN(prd_key)) AS abb_prd_key,
			prd_nm,
		...
		-- Loading the silver.crm_sales_details table
		SET @start_time = GETDATE();
